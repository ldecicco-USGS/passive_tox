...
join_everything_fnx <- function(chemicalSummary){
  ...
  panther <- data.table::fread(here("panther_data/joined_genes.csv")) %>% 
    data.frame() %>% 
    select(geneSymbol = gene_abbr,
           pathway_accession,
           pathway_name) %>% 
    left_join(gene,  by="geneSymbol")
  ...
  join_everything <- chemicalSummary %>% 
    left_join(select(AOP_crosswalk, endPoint, AOP_shortname),
              by="endPoint") %>% 
    left_join(gene, by="endPoint") %>% 
    left_join(panther, by=c("endPoint","geneSymbol")) %>% 
    select(endPoint, geneSymbol, AOP_shortname, pathway_name) %>% 
    distinct() %>% 
    group_by(endPoint) %>% 
