...
We will look at mixtures based on unique chemical/endpoint combinations. The way we prioritize mixutres is summing EARs for each endpoint, and then only look at endpoints that have at least an sumEAR of 0.001. Then, we go back and figure out which mixtures occur at at least 10 sites. The only mixtures that fit those 2 critiera are:
...
df_lu <- open_land_use() %>% 
  filter(site %in% unique(chemicalSummary$site))
