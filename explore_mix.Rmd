...
check_mix <- top_mixes %>% 
  ...
  bind_rows(select(top_mixes_per_ep, chemicals = contr_chems_st)) %>% 
  distinct()
...
Reduce the variables by those that have a corrolation coefficient >0.95 or <-0.95.
...
for(i in seq_along(lu_vars)){
  ...
  if(check_var %in% exclude){
    ...
    check_cor <- names(which(M[,check_var] > 0.95 | 
                        M[,check_var] < -0.95))
...
The variables left by the 0.95 threshold:
...
df_lu_filtered <- bind_cols(df_lu[,"site"], df_lu_filtered)

x <- data.frame(matrix(NA,
                  nrow = length(names(df_lu_filtered)), 
                  ncol = nrow(check_mix)))

names(x) <- check_mix$chemicals
...
for(i in seq_len(nrow(top_mixes_per_ep))){
  ...
  sub_df$sumEAR_offset <- sub_df$sumEAR_offset +
    0.5*min(sub_df$sumEAR_offset[sub_df$sumEAR_offset > 0], na.rm = TRUE)
