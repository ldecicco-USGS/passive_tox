...
form2_lm <- formula(log10(sumEAR_shifted) ~ Urban + Agriculture + Urban^2 + Agriculture^2)
...
form_survival_log <- formula(survival::Surv(log10(lowEAR),
                                log10(highEAR),
                                 type="interval2") ~
                                 Urban + Agriculture +
                                 Urban^2 + Agriculture^2)
...
for(i in seq_len(nrow(top_mixes))){
  ...
  cat("\n### Linear\n")
  x_df <- plot_lm(form_survival, sub_df)
  
  cat("\n")
  cat("\n### Log\n")
  x_df2 <- plot_lm(form_survival_log, sub_df, 
                  log=TRUE)
  cat("\n")
...
fx <- flextable(x[,1:3])
fx <- width(fx, width = 2.5)
cat("\n##### First 3")
fx
cat("\n")
```

```{r pickAndChoose2, results="asis"}
fx2 <- flextable(x[,4:5])

fx2 <- width(fx2, width = 2.5)
cat("\n##### Next 2")
fx2
cat("\n")
```
...
for(i in seq_len(nrow(top_mixes))){
  ...
  form <- reformulate(termlabels = attr(terms(new_form),"term.labels"),
              response = 'survival::Surv(lowEAR,
                                      highEAR,
                                      type="interval2")')

  x_df <- plot_lm(form, sub_df = sub_df, survival = TRUE)
  cat("\n")
  ...
  new_form2 <- reformulate(termlabels = attr(terms(new_form2),"term.labels"),
              response = 'survival::Surv(log10(lowEAR),
                                      log10(highEAR),
                                      type="interval2")')
  
  x_df2 <- plot_lm(new_form2, sub_df,
                  sumEAR = "sumEAR",
                  log=TRUE, survival = TRUE)
...
for(i in seq_len(nrow(top_mixes_per_ep))){
  ...
  if(is.null(new_form)){
    ...
    new_form <- reformulate(termlabels = attr(terms(new_form), "term.labels"), 
    response = 'survival::Surv(lowEAR,
                               highEAR,
                               type="interval2")')
    x_df <- plot_lm(new_form, sub_df, survival = TRUE)
  }
  ...
  if(is.null(new_form_log)){
    ...
    new_form_log <- reformulate(termlabels = attr(terms(new_form_log), "term.labels"), 
    response = 'survival::Surv(log10(lowEAR),
                               log10(highEAR), 
                               type="interval2")')
